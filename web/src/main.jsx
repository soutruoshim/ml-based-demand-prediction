...
function App() {
  ...
  return <main>
    ...
    <section className="upload-card"><form onSubmit={submit}><label className="drop"><input type="file" accept=".csv,text/csv" onChange={e => setFile(e.target.files[0])}/><span className="icon">↥</span><strong>{file ? file.name : t.choose}</strong><small>{t.fileHelp}</small></label><div className="actions"><button disabled={loading}>{loading ? t.training : t.analyze}</button><a href={`${API}/api/template`}>{t.template}</a></div></form><p className="hint">{t.required}</p>{error && <div className="error">{error}</div>}</section>
